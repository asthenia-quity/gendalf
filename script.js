...
if(document.documentElement.clientWidth){
    document.querySelector('.burger_btn').addEventListener('click', ()=>{
        ...
        const pic1 = document.querySelector('.toggle1');
        const pic2 = document.querySelector('.toggle2');
...
$('.life-lent__cards').slick({
  ...
  document.getElementById('file-upload').addEventListener('change', function(event) {
    ...
    const fileLabelSpan = document.getElementById('file-label');
    ...
    if (file) {
        ...
        fileLabelSpan.textContent = fileName;
        fileLabelSpan.style.color = '#666666'; 

        const closeButton = document.createElement('span');
        closeButton.classList.add('close-btn');
        closeButton.addEventListener('click', function() {
            fileInfoDiv.style.display = 'none';
            fileLabelSpan.textContent = 'Прикрепить резюме';
            fileLabelSpan.style.color = '#9BCC37';
            const img = document.createElement('img');
            img.classList.add('btnform');
            img.src = 'images/btnform.png';
            img.alt = '';
            fileLabelSpan.appendChild(img);
            document.getElementById('file-upload').value = '';
        });

        fileLabelSpan.appendChild(closeButton);
    }
});
